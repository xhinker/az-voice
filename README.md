...
VoxCPM2 is one of the best open-source TTS models — but using it directly is painful. Feed it long text and it breaks at the 8192-token limit. Want voice cloning? Fight with file formats and tensor shapes. Need streaming? The model yields numpy arrays, not HTTP responses.

**az-voice fills the gaps the base model leaves open:**

1. **Text Splitting** — Splits long English/Chinese text at sentence boundaries, merges to target duration (15s default), handles mixed-language content.
2. **Streaming Without Blips** — Raw PCM via SSE, with a smoother that crossfades only when discontinuities are detected.
3. **Library Wrapper** — Engine class handles loading, device placement, VRAM cleanup, format conversion, and prompt cache reuse for voice cloning.
4. **OpenAI-Compatible API** — Drop-in replacement for `/v1/audio/speech`. Swap the base URL and your code works.
5. **Web UI** — Dark theme, batch and streaming tabs, reference audio upload. No build step, no npm.

[Read the full story on Medium](https://medium.com/p/974b371b1a64)
